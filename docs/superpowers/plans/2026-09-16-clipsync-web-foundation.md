...
- Produces:
  - `deriveSharedKey(myExchangePrivateKey: CryptoKey, peerExchangePublicKeyJwk: string): Promise<CryptoKey>` — an AES-256-GCM `CryptoKey`, derived via X25519 ECDH between this device and its one paired peer, then HKDF. The screens plan calls this once, right after pairing completes (using the `peerDevices[0].publicKeyExchangeJwk` the pairing-complete response already returns per Task 4), and again on every app launch once a peer identity is known.
